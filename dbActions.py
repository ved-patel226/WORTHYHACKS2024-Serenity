...
class Posts(Base):
    ...
    def __repr__(self):
        return f"<Posts(title='{self.title}', body='{self.body}', upvote={self.upvote}, location='{self.location}')>"
...
class DbActions:
    ...
    def append(self, values: list, debug=False) -> None:
        ...
        try:
            if len(values) == 4 and isinstance(values[0], str) and isinstance(values[1], str) and isinstance(values[2], int) and isinstance(values[3], str):
                db_value = Posts(title=values[0], body=values[1], upvote=values[2], location=values[3])
    ...
    def wipe_except_posts(self) -> None:
        ...
        print("All tables except 'posts' have been dropped.")
...
if __name__ == "__main__":
    ...
    db.read_table('petitions', debug=True)